...
const databaseIcone = {
    sc: { // STAR CITIZEN
        ...
        categorie: {
            ...
            "OP FPS": [
                { nome: "FPS General", file: "icone amiche/OP_FPS/SQ_FPSterra.png" },
...
window.cambiaSetIcone = () => {
    ...
    // Mostra la tendina dei sistemi SOLO se è selezionato Star Citizen
    if (filtroSistema) {
        if (giocoSelezionato === 'sc') {
            filtroSistema.style.display = 'block';
        } else {
    ...
    for (const [nomeCategoria, listaIcone] of Object.entries(setCorrente.categorie)) {
        if (listaIcone.length > 0) {
            ...
            // Il tuo CSS applicherà in automatico il colore #88a0b0 e l'estensione su 3 colonne
            let titolo = document.createElement('div');
            titolo.innerText = nomeCategoria;
            container.appendChild(titolo);

            // Crea i bottoni e li butta direttamente nel grid-container
            // Il tuo CSS li disporrà perfettamente a 3 a 3 con l'effetto hover
            listaIcone.forEach(icona => {
                let btn = document.createElement('button');
                btn.className = "btn-icona";
                btn.innerText = icona.nome;

                // Quando l'utente clicca sul bottone, spawna l'icona usando la nostra nuova funzione
                btn.onclick = () => {
                    let fullPath = setCorrente.basePath + icona.file;
                    fullPath = fullPath.replace('.png', ''); // Pulisce il .png per retrocompatibilità
                    
                    // Determina in automatico se l'icona è una squadra schierata o un semplice POI/Segnalino
                    let tipo = (nomeCategoria.includes('FPS') || nomeCategoria.includes('VOLO') || nomeCategoria.includes('Industriali') || nomeCategoria.includes('Tattica')) ? 'squadra' : 'poi';
                    
                    if(typeof creaElemento === 'function') creaElemento(fullPath, tipo);
                };
