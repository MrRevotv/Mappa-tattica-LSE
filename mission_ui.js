...
window.apprendiMappaDaUpload = (fileInput) => {
    ...
    let idMappa = nomeFileCompleto.split('.').slice(0, -1).join('.'); // Rimuove .png/.jpg -> "Ghost Hollow"
    if (!idMappa) idMappa = nomeFileCompleto;

    // Nome leggibile per la UI (spazi e maiuscole), costruito prima della sanitizzazione
    let nomePulito = idMappa.replace(/[-_]/g, ' ').replace(/\b\w/g, char => char.toUpperCase());

    // ID sanitizzato: spazi → underscore, allineato a come il server salva la cartella
    idMappa = idMappa.replace(/\s+/g, '_');
